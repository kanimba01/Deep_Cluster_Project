...
class IndexedDataset(Dataset):
    ...
    def __init__(self, dataset, transform=None, limit=None):
        ...
        self.image_files = sorted(os.listdir(dataset))[:limit]
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.dataset, self.image_files[index])
...
def get_loader(image_dir, batch_size=128, limit=None):
    """ Prepare and return a data loader. """
    # raw_dataset = datasets.MNIST(root='./data', train=True, download=True)
    dataset = IndexedDataset(image_dir, transform=get_transform(), limit=limit)
